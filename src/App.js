...
class App extends Component {
	...
	handleChange(checked) {
		this.state.current_theme == SunnyTheme
			? this.setState({ current_theme: DarkTheme, checked })
			: this.setState({ current_theme: SunnyTheme, checked });
	}

	render() {
		const { current_theme } = this.state;
		return (
			<div style={{ backgroundColor: current_theme.backgroundColor }}>
				...
				<div style={{ minHeight: '100vh' }}>
					{' '}
					<SubScreen.LandingScreen theme={current_theme} />{' '}
				</div>
				<LineSeparator color={current_theme.color} />
				<div style={{ minHeight: '100vh' }}>
					{' '}
					<SubScreen.About theme={current_theme} />{' '}
				</div>
				{/* <LineSeparator color={current_theme.color}/> */}

				{/* <SubScreen.Projects theme={current_theme}/> */}
				<div style={{ marginTop: '30px' }}>
					<SubScreen.Footer theme={current_theme} />{' '}
				</div>
			</div>
		);
	}
